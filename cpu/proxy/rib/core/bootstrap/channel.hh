...
protected:
  ...
  Result<sockaddr> try_recv(ByteBuffer &buf, const double to_usec = 1000000) {
    ...
    int sec = static_cast<int>(to_usec / 1000000);
    int usec = static_cast<int>(to_usec) % 1000000;
...
class RecvChannel : public AbsChannel {
  ...
  RecvChannel(int port, const std::string &addr)
    ...
    if ((getaddrinfo(addr.c_str(), std::to_string(port).c_str(), &hints,
                     &servinfo)) != 0) {
      goto err;
    }
