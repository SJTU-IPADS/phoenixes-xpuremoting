...
public:
  ...
  Result<std::string> send_normal(const ReqDesc &desc,
    ...
    if (payload.compare_add == 0 && payload.swap == 0) {
      sr.wr.rdma.remote_addr = remote_mr.buf + payload.remote_addr;
      sr.wr.rdma.rkey = remote_mr.key;
    } else {
      sr.wr.atomic.remote_addr = remote_mr.buf + payload.remote_addr;
      sr.wr.atomic.compare_add = payload.compare_add;
      sr.wr.atomic.swap = payload.swap;
